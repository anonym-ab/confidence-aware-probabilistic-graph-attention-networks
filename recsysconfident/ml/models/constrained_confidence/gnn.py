...
def get_gnn_constrained_model_and_dataloader(info: DatasetInfo):
...
class Gnn(TorchModel):
    ...
    def forward(self, users_ids, items_ids):
        ...
        dot_product = (u_graph_emb * i_graph_emb)
        ...
        sim = dot_product.sum(dim=1).squeeze() / (u_norm * i_norm)  # Compute cosine similarity

        x = F.leaky_relu(self.fc1(dot_product))
        x = self.dropout(x)
        x = self.fc2(x)
        ...
        return prediction.squeeze(), confidence
