...
def get_mf_not_reg_conf_constrained_model_and_dataloader(info: DatasetInfo):
...
class MFNonRegularizedModel(TorchModel):
    ...
    def __init__(self, num_users, num_items, num_factors, max_error: float = 5):
    ...
    def forward(self, user, item):
        ...
        sim = dot_product / (u_norm * i_norm) # cosine similarity
        ...
        confidence = torch.abs(sim*w_conf_ui - sim.mean()*mu) #confidence estimation
    ...
    def loss(self, data, device):
        ...
        conf_penalty = torch.abs(error_abs - self.max_error * (1.0 - confidence))
        confidence_loss = torch.maximum(torch.zeros_like(error_abs), conf_penalty).mean()

        loss = criterion + confidence_loss * 0.5
